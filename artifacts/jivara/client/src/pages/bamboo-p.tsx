...
import img1 from "@assets/FB_IMG_1778705215362_1778705306652.jpg";
import img2 from "@assets/FB_IMG_1778705160256_1778705306725.jpg";
import img3 from "@assets/FB_IMG_1778705152857_1778705306745.jpg";
import img4 from "@assets/FB_IMG_1778705142824_1778705306770.jpg";
import img5 from "@assets/FB_IMG_1778705166933_1778705306786.jpg";
...
const SOCK_IMAGES = [img1, img2, img3, img4, img5];
...
export default function BambooPPage() {
  ...
  const orderMutation = useMutation({
    mutationFn: async () => {
      ...
      const sessionId = safeStorage.getItem("bamboo-p-session") || ("bamp-" + Math.random().toString(36).substring(7));
      safeStorage.setItem("bamboo-p-session", sessionId);
      return await apiRequest("POST", "/api/orders", {
        ...
        city,
        notes: `مصدر: Bamboo-P | الكمية: ${qty} بوكس`,
        totalAmount: String(PRICE_IQD * qty),
        landingPage: "/bamboo-p",
        fbclid: getFbclid(),
    ...
    onSuccess: async (data: any) => {
      ...
      const orderId = __r?.id || __r?.order?.id || `bamp-${Date.now()}`;
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-white font-sans">
      ...
      <header className="bg-black text-white py-3 px-4">
        <div className="max-w-xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="text-xl font-black tracking-wide text-white">🧦 بامبو بريطاني</span>
          </div>
          <div className="flex items-center gap-1 text-xs text-gray-300">
            <MapPin className="w-3 h-3" />
            <span>يوصل لكل العراق</span>
          </div>
        </div>
      </header>
      ...
      <div className="px-4 py-4 max-w-xl mx-auto">
          ...
          <div className="grid grid-cols-5 gap-2 mb-3">
            {COLORS.map((color, i) => (
              <div key={i} className="flex flex-col items-center gap-1">
                <div className="w-10 h-10 rounded-full border-2 border-gray-600 shadow-inner" style={{ backgroundColor: color.hex }} />
        ...
        <div className="bg-gradient-to-r from-black to-gray-800 text-white rounded-2xl p-4 mb-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-bold">التوصيل لكل محافظات العراق</p>
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-2xl p-5 mb-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <button type="submit" disabled={orderMutation.isPending}
              className={`w-full text-white font-black text-lg py-4 rounded-2xl transition-all duration-500 active:scale-95 shadow-lg disabled:opacity-80 ${orderMutation.isPending ? "bg-blue-500" : isFormReady ? "bg-green-500 hover:bg-green-600 scale-[1.02]" : "bg-black hover:bg-gray-900"}`}>
              {orderMutation.isPending ? `جاري إرسال الطلب... ${progress}%` : isFormReady ? "👆 اضغط هنا لإرسال الطلب" : "اطلب الآن — الدفع عند الاستلام"}
            </button>
          </form>
        </div>
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-gray-400 text-xs mt-2">07819966698</p>
      ...
      <footer className="bg-gray-900 text-gray-400 text-center py-4 px-4 text-xs">
        <p className="font-bold text-white text-sm mb-1">جوارب بامبو البريطانية</p>
        <p>يوصل لكل العراق • 07819966698</p>
      </footer>
    </div>
  );
}
